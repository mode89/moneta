...
* `just lint` checks every JavaScript file in `web/` with ESLint (`web/eslint.config.js`, the recommended rule set plus per-area globals) and with Prettier in check mode. `just format` applies Prettier. `web/.prettierrc` fixes indentation at 2 spaces, forbids tabs and sets the line width to 80 columns, and turns `embeddedLanguageFormatting` off because it otherwise reformats the `html` tagged templates as HTML and mangles the Solid markup. `web/.prettierignore` excludes `index.html`, whose layout is hand-made around the import map, the custom CSS and the `%VERSION%` placeholder.
