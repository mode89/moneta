...
* `just build` builds the web app into `build/web/dist` (`build-web`), then packages the APK (`build-android`, which first runs `sync` for `npx cap sync android`). Either half can be run alone.
...
The unit tests cover the domain layer of `main.js` (validation, form conversion, stored JSON, store actions, import); the Playwright suite covers the Solid components and the flows through them (adding, editing, deleting, grouping and totals, blurred amounts, persistence and reload, import/export in both the browser and the native branch). Between them a UI change should be provable without a device, but what the system does with a shared file, a file picker or a display cutout still needs a device.
