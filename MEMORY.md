...
- UI changes are verified in a real browser via `scripts/e2e`, the Playwright suite in `web/e2e/`; the Node unit tests cover only the domain layer. Why: nothing else catches wiring errors with no compile step.
...
- `android/src/main/assets/web/` holds build output that can lag `web/`, so `scripts/serve` and the installed APK show a stale UI until `scripts/build` runs; a bug seen "in the app" may be a bug in old assets.
- `scripts/build` and `web/e2e/server.js` copy and serve `bootstrap.css`, `file_download.svg` and `file_upload.svg`, which `web/index.html` no longer references.
...
- The app is zero-build — Solid's `html` tagged template plus an import map, no esbuild or Vite. Why: `scripts/build` stays copy-only and `scripts/serve` iteration is instant.
...
- Fraunces and DM Sans load from Google Fonts at runtime, so a device with no network falls back to a system serif and sans. Why: self-hosting them would make `scripts/build` copy font files.
- The Playwright suite serves the app from its own `web/e2e/server.js` rather than `scripts/serve`. Why: `scripts/serve` publishes the *built* assets, which would make the UI suite depend on a full Android build.
...
- The UI suite runs every spec twice, in Asia/Kolkata and America/New_York, as `scripts/test` does. Why: the UI is full of dates, and a day that slips only west of the meridian must not pass unnoticed.
...
- ? The Gradle packaging step is unverified: `nix-shell --run "scripts/build"` has not been run against the current `web/` and `scripts/build`.
